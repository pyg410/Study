...
switch문에 새 코드를 행을 추가하려면 그렇게 여기저기에 존재하는 switch문을 전부 찾아서 수정해야 한다.

해결책은 다형성을 이용하는 것이다. 즉, 재정의를 하면 된다.

재정의 할 위치는 분류부호 값이 들어있는 메서드나 클래스부분이다.

1. 분류 부호 값이 들어있는 메서드를 추출한다.
2. switch문을 메서드로 빼낸 후 메서드를 재정의할 클래스로 옮긴다.
3. 분류 부호를 하위클래스로 전환한다.(선택1)
4. 분류 부호를 상태/전략 패턴으로 전환한다.(선택2)
5. 상속구조로 만든 후에 조건문을 재정의로 전환한다.

해당 해결책의 주의할 점은 case문이 2~3개밖에 없고 나중에 모든 case문을 수정할 일이 없을 것 같다면 재정의는 과할 수 있다. 
이 경우 메개변수를 메서드로 전환을 적용하는 게 좋다.

여러 case문 중에 하나가 null이라면 null검사를 null객체에 위임을 실시하면 좋다.

## 평행 상속 계층

기능의 산재의 특수한 상황이다.

이 문제점은 한 클래스의 하위클래스를 만들 때마다 매번 다른 클래스의 하위클래스도 만들어야 한다.

서로 다른 두 상속계층의 클래스면 접두어가 같으면 이 문제를 의심할 수 있다.

중복 코드 제거를 위해 한 상속 계층의 인스턴스가 다른 상속계층의 인스턴스를 참조하게 만들면 된다.

메서드 이동, 필드 이동을 실시하면 된다.

## 직무유기 클래스

하나의 클래스를 작성할 땐, 유지관리와 이해하기 위한 비용이 추가된다.

즉, 비용만큼의 기능을 수행하지 못하는 클래스는 없애야 한다.

이럴 땐 계층 병합을 하면 된다.

클래스 내용 직접 삽입, 모듈내용 직접삽입 기법을 사용하면 된다.

## 막연한 범용 코드

조만간 이런 기능이 필요하겠구만 하는 막연한 생각에 아직 필요없는 기능을 수행하고자 코드를 작성하면 막연한 범용코드가 생긴다.

별다른 기능이 없는 클래스나 모듈이 있다면 계층병합.
불필요한 위임을 제거하려면 클래스 내용 직접 삽입을 한다.
메서드에 사용되지 않는 매개변수가 있으면 매개변수 제거를 실시.
메서드명이 이상하다면 메서드명 변경을 실시한다.

## 임시필드

어떤 객체 안에 인스턴스 변수가 특정 상황에서만 할당되는 경우가 있다.

떠돌이 변수들이 저장될 클래스를 만든다.

작성한 클래스에 변수와 관련된 코드를 전부 넣는다.

null검사는 널객체에 위임한다.

임시 필드(변수)는 복잡한 알고리즘에 여러 변수를 사용해야 할 때 풍긴다.

인스턴스 변수와 그 변수를 사용하는 메서드에 대해 메서드 전부에 대해 클래스 추출을 적용하면, 메서드 객체가 생성된다.
